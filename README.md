...
Jojo 是一个基于 Claude CLI 的 Web 聊天应用，采用终端/黑客风格界面，支持多会话管理、流式响应、图片上传、实时任务进度监控和多用户权限控制。

## 功能特性

- **终端风格界面** — 深色终端背景 + 等宽字体 + CRT扫描线效果，沉浸式黑客体验
- **实时流式对话** — 基于 Claude CLI，支持 SSE 流式输出，打字机效果逐字显示
- **实时任务进度框** — 左下角常驻科技蓝进度监控面板，显示计时器、进度条、字符统计等
